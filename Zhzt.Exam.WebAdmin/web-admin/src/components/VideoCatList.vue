...
<template>
    ...
    <VideoCatAddDlg ref="quesTypeAddDlgRef" :type="editType" :reload="reload" :allCategories="allCategories" />
</template>
...
import VideoCatAddDlg from './VideoCatAddDlg.vue'
...
export default {
    ...
    setup(props) {
        ...
        const loadVideoCategories = () => {
            let url = `/microclasslib/videocategory/page/tree?pageIndex=${state.pageIndex}&pageSize=${state.pageSize}`
            axios.get(url)
                .then(res => {
        ...
        const loadAllVideoCategories = () => {
            ...
            axios.get(url)
                .then(res => {
                    state.allCategories = quesTypeAddDlgRef.value.convertQuestionType(res.data)
        ...
        const handleDeleteOne = (id) => {
            axios.delete(`/microclasslib/videocategory/delete?id=${id}`)
                .then(res => {
        ...
        const handleEdit = (row) => {
            state.editType = 'edit'
            state.allCategories = quesTypeAddDlgRef.value.disableSelf(state.allCategories, row.id)
        ...
        const handleCreate = () => {
            state.editType = 'add'
            state.allCategories = quesTypeAddDlgRef.value.unDisableAll(state.allCategories)
