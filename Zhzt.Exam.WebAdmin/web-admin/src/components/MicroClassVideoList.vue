<template>
    ...
    <el-table :data="tableData" style="width: 100%" :border="true" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="所属类别" >
            <template #default="scope">
                <el-tag>{{ scope.row.category?.name ?? '未分类' }}</el-tag>
            </template>
        </el-table-column>
    ...
    <el-pagination background layout="prev, pager, next" class="center-pagination" v-model:currentPage="pageIndex"
     ...
     <VideoPreviewDlg ref="videoPreDlgRef"/>
</template>
...
import VideoPreviewDlg from './VideoPreviewDlg.vue'

export default {
    name: "MicroClassVideoList",
    components:{VideoPreviewDlg},
    props: {
        ...
        allVideoCategories: { type: Array, default: () => [] }
    },
    setup(props) {
        ...
        const videoPreDlgRef = ref(null)

        // 挂载方法
        onMounted(() => {
            filterMicroClassVideos()
        })

        // 分页加载问题列表 支持筛选
        const filterMicroClassVideos = (filter) => {
            let url = `/microclasslib/microclassvideo/filterpage`;
            let filterData = {};
            if (filter) {
                filterData = filter;
            }
        ...
        const handleDeleteOne = (id) => {
            axios.delete(`/microclasslib/microclassvideo/delete?id=${id}`)
                .then(res => {
        ...
        // 转换科目列表支持cascader
        const convertVideoCategoty = (questionList) => {
            if (questionList) {
                return questionList.map((val) => {
                    ...
                    rObj['value'] = val.id
                    rObj['label'] = val.name
                    rObj['children'] = convertVideoCategoty(val.child)
                    rObj['disabled'] = false
                    return rObj
                })
            }
